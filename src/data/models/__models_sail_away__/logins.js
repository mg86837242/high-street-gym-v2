...
export function getLoginByUsername(username) {
    return db_conn.query('SELECT * FROM logins WHERE login_username = ?', [username]);
}

export function createLogin(username, password) {
    return db_conn.query(`INSERT INTO logins (login_username, login_password) VALUES (?, ?)`, [username, password]);
}
