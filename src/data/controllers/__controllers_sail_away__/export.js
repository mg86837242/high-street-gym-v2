...
exportController.get('/cruise-list', async (req, res) => {
	...
	const xml = ejs.render(fs.readFileSync('./src/backend/xml/cruise-list.xml.ejs').toString(), {
		cruises: cruises,
	});

	// Send XML as download
	res.status(200)
		.header('Content-Disposition', 'attachment; filename="cruise-list-export.xml"')
